...
from sema4ai.di_client.document_intelligence_client.models.raw_document_content_all_of_raw_content import (
...
COMPUTABLE_DOCUMENT_TYPE_FIELDS = [
    COMPUTED_TOTAL_NUMBER_OF_INVOICES_IN_REMITTANCE,
    COMPUTED_TOTAL_INVOICE_AMOUNT_BY_FACILITY_TYPE,
]
...
class ValidationProcessor(DocumentProcessorBase):
    def __init__(
        ...
        self.agent_insight_context_manager: ValidationAgentContextManager = (
            agent_context_manager
        )

        self.di_utility: DocumentIntelligenceUtility = DocumentIntelligenceUtility(
            self.source_document.document_type,
            self.source_document.document_format,
            self.agent_insight_context_manager,
        )
        self.table_extractor = TableExtractor(
            self.source_document, self.agent_insight_context_manager
        )

    @ValidationAgentContextManager.track_method_execution(
        method_name="extract_and_structure_content"
    )
    def extract_and_structure_content(
        self, raw_document: RawDocumentContent
    ) -> ExtractionResult:
        """
        ...
        """
        with self.agent_insight_context_manager.phase_context(
            ProcessingPhase.EXTRACTION
        ):
            self.logger.info(
                f"Starting extraction process for document: {self.source_document.document_name}"
            )
            ...
            extraction_result: Dict[str, Any] = self._extract_tables(
            ...
            invoice_details_df = extraction_result.get(
            ...
            standardized_non_tabular_data = (
                self.di_utility.standardize_non_tabular_to_doc_type_schema(
                    non_tabular_data
                )
            )

            invoice_details_table_name = self._get_remittance_table_names().get(
                INVOICE_TABLE_KEY, None
            )
            subtotals_table_name = self._get_remittance_table_names().get(
                SUBTOTALS_TABLE_KEY, None
            )
            ...
            standardized_invoice_details = (
                self.di_utility.standardize_tabular_to_doc_type_schema(
                    {"table_data": invoice_details_df.to_dict("records")},
            ...
            converted_non_tabular_data, non_tabular_validation = (
                self.di_utility.validate_and_clean_non_tabular(
                    standardized_non_tabular_data,
            ...
            converted_invoice_details_df, invoice_details_validation = (
                self.di_utility.validate_and_clean_tabular(
            ...
            converted_summary_df, summary_validation = (
                self.di_utility.validate_and_clean_tabular(
            ...
            self.agent_insight_context_manager.add_validation_results(
            ...
            clean_invoice_details_dict = clean_any_object_safely(
                converted_invoice_details_df.to_dict("records")
            )
            clean_summary_df_dict = clean_any_object_safely(
                converted_summary_df.to_dict("records")
            )
            extracted_content = {
            ...
            self.logger.info(
                f"Completed extraction process for document: {self.source_document.document_name}"
            )

            return extraction_result

    @ValidationAgentContextManager.track_method_execution(
        method_name="transform_and_enrich_content"
    )
    def transform_and_enrich_content(
        self, extracted_document_content: ExtractedDocumentContent
    ) -> TransformationResult:
        """
        ...
        with self.agent_insight_context_manager.phase_context(
            ...
            document_id, invoice_details_df, summary_df, non_tabular_data = (
                self._setup_transformation(extracted_document_content)
            )

            # Compute Total Number of Invoice Line Items Field
            total_invoice_line_items = self._compute_total_invoice_line_items(
                invoice_details_df
            )

            # Extract subtotals for each Facility Type
            invoice_subtotals_by_facility_type = (
                self._compute_invoice_totals_by_facility_type(invoice_details_df)
            )
            ...
            clean_invoice_details_dict = clean_any_object_safely(
                invoice_details_df.to_dict("records")
            )
            clean_summary_df_dict = clean_any_object_safely(
            ...
            self.agent_insight_context_manager.add_event(
                ...
                "Completed content transformation and enrichment",
            )

            self.logger.info(
                "transformed_document_content being return as part of Trasnforemd REsult is: "
                + serialize_any_object_safely(transformed_document_content)
            )
    ...
    @ValidationAgentContextManager.track_method_execution(
        method_name="validate_and_finalize_content"
    )
    def validate_and_finalize_content(
        ...
        with self.agent_insight_context_manager.phase_context(
            ...
            self.agent_insight_context_manager.add_event(
                "Validation Start", "Beginning content validation"
            )
            ...
            self._calculate_validation_metrics_and_save_to_agent_context(
                validation_results
            )
            ...
            self.agent_insight_context_manager.add_event(
                "Validation Complete", "Completed content validation"
            )
    ...
    def _validate_total_invoices(self, transformed_doc, validation_results):
        total_invoices = int(
            transformed_doc.transformed_content["fields"]["Total Invoices"]
        )
        computed_total = transformed_doc.transformed_content["computed_fields"][
            "total_invoice_line_items"
        ]
        ...
        validation_results.add_result(
            ...
            if total_invoices != computed_total
            else ValidationSeverity.INFO,
            details={
                "extracted_total": total_invoices,
                "computed_total": computed_total,
            },
        )

    def _validate_facility_type_subtotals(self, transformed_doc, validation_results):
        computed_subtotals = transformed_doc.transformed_content["computed_fields"][
            "invoice_subtotals_grouped_by_facility_type"
        ]
        ...
        message = (
            "All facility type subtotals match"
            if all_subtotals_match
            else f"Subtotals do not match for the following facility types: {', '.join(mismatched_types)}"
        )
    ...
    def _validate_total_payment(self, transformed_doc, validation_results):
        extracted_total_payment = float(
            transformed_doc.transformed_content["fields"]["Total Payment Paid"]
        )
        computed_total_payment = transformed_doc.transformed_content["computed_fields"][
            "computed_total_amount_paid"
        ]
        ...
        validation_results.add_result(
            ...
            details={
                "extracted_total_payment": extracted_total_payment,
                "computed_total_payment": computed_total_payment,
                "difference": extracted_total_payment - computed_total_payment,
            },
        )
    ...
    def _validate_discounts_and_charges(self, transformed_doc, validation_results):
        ...
        computed_discounts = sum(
            float(item.get("Discounts Applied", 0)) for item in invoice_details
        )
        computed_charges = sum(
            float(item.get("Additional Charges", 0)) for item in invoice_details
        )
        ...
        self.logger.info(
            f"Discounts validation: {'Passed' if discounts_match else 'Failed'}"
        )
        self.logger.info(
            f"Charges validation: {'Passed' if charges_match else 'Failed'}"
        )
    ...
    def _calculate_validation_metrics_and_save_to_agent_context(
        ...
        passed_validations = sum(
            1 for result in validation_results.results if result.passed
        )
    ...
    def _compute_invoice_totals_by_facility_type(
        ...
        self.logger.info(
            f"Invoice Amount is: {df['Invoice Amount']}. Type is {type(df['Invoice Amount'])}"
        )
        self.logger.info(
            f"Discounts Applied is: {df['Discounts Applied']}. Type is {type(df['Discounts Applied'])}"
        )
    ...
    def _compute_total_amount_paid(self, df: pd.DataFrame) -> float:
        ...
        total_amount_paid = (
            df["Amount Paid"]
            .apply(lambda x: self.logger.info(f"Adding: {x}") or x)
            .sum()
        )
    ...
    def _add_document_configs_to_agent_context(self):
        ...
        doc_type_config = {
            "non_tbl_fields": [
                field.dict()
                for field in self.source_document.document_type.non_tbl_fields
            ],
            "tbl_fields": [
                table.dict() for table in self.source_document.document_type.tbl_fields
            ],
        }
        doc_format_config = {
            "non_tbl_fields_mapping": [
                mapping.dict()
                for mapping in self.source_document.document_format.non_tbl_fields_mapping
            ],
            "tables": [
                table.dict() for table in self.source_document.document_format.tables
            ],
    ...
    def _setup_transformation(
        self, extracted_document_content: ExtractedDocumentContent
    ) -> Tuple[str, pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
        document_id = extracted_document_content.document_id
        invoice_details_data: List[Dict[str, Any]] = (
            extracted_document_content.extracted_content["invoice_details"]
        )
        summary_data: List[Dict[str, Any]] = (
            extracted_document_content.extracted_content["summary"]
        )
        non_tabular_data: Dict[str, Any] = extracted_document_content.extracted_content[
            "fields"
        ]
        ...
        self.logger.info(
            f"Invoice Details - Rows: {invoice_details_rows}, Columns: {invoice_details_columns}"
        )
        ...
        self.agent_insight_context_manager.add_event(
            ...
            "Beginning content transformation and enrichment",
            {
                ...
                "non_tabular_data_metrics": {"num_fields": num_non_tabular_fields},
            },
        )
    ...
    def _add_facility_type_and_process_co2(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "CO2 Supplementation" in df.columns:
            df["CO2 Supplementation"] = (
                df["CO2 Supplementation"].replace(",", "").astype(float)
            )
    ...
    def get_facility_type(self, invoice_reference):
        ...
        if len(parts) >= 3:
            ...
            facility_name_mapping_str = self.di_utility.get_custom_config_value(
                CONFIG_FACILITY_NAME_MAPPING_KEY, "{}"
            )
            ...
            facility_type = facility_name_mapping.get(
                f"INV-{facility_type_abbr}", "Unknown"
            )
    ...
    def _log_unknown_facility_type(self, facility_id: str):
        ...
        unknown_facilities = self.agent_insight_context_manager.get_context(
            "unknown_facilities", []
        )
        unknown_facilities.append(facility_id)
        self.agent_insight_context_manager.add_context(
            "unknown_facilities", unknown_facilities
        )
    ...
    def _extract_tables(
        ...
        extraction_result = self.table_extractor.extract_tables_from_pages(
            raw_content_pages
        )

        # Log event and update metrics for invoice details
        if "invoice_details" in extraction_result and isinstance(
            extraction_result["invoice_details"], pd.DataFrame
        ):
        ...
        if "summary" in extraction_result and isinstance(
            extraction_result["summary"], pd.DataFrame
        ):
        ...
        if "metrics" in extraction_result:
            ...
            self.agent_insight_context_manager.update_metrics(
                {"Metrics on Table Extracted": metrics}
            )

        return extraction_result

    @ValidationAgentContextManager.track_method_execution(
        method_name="_calculate_co2_supplementation"
    )
    def _calculate_co2_supplementation(
        self, df: pd.DataFrame, document_format: DocumentFormat
    ) -> Dict[str, float]:
        self.agent_insight_context_manager.add_event(
            "CO2 Supplementation Calculation Start",
            "Beginning calculation of CO2 supplementation",
        )
        facility_type_mapping = self.di_utility.get_custom_config_value(
            CONFIG_FACILITY_NAME_MAPPING_KEY, {}
        )
        greenhouse_prefix = next(
            k for k, v in facility_type_mapping.items() if v == "Greenhouse Complexes"
        )

        greenhouse_df = df[df["Invoice Reference"].str.startswith(greenhouse_prefix)]
        co2_supplementation = (
            greenhouse_df.groupby("Facility ID")["CO2 Supplementation (kg)"]
            .sum()
            .to_dict()
        )

        self.agent_insight_context_manager.add_event(
            "CO2 Supplementation Calculation Complete",
            f"Calculated CO2 supplementation for {len(co2_supplementation)} greenhouse facilities",
        )
        return co2_supplementation

    @ValidationAgentContextManager.track_method_execution(
        method_name="_apply_co2_policy"
    )
    def _apply_co2_policy(
        self, co2_data: Dict[str, float], policy: Dict[str, Any]
    ) -> Dict[str, float]:
        self.agent_insight_context_manager.add_event(
            "CO2 Policy Application Start", "Beginning application of CO2 policy"
        )
        adjustments = {}
        for facility, amount in co2_data.items():
            if amount > policy.get("threshold", float("inf")):
                ...
                adjustments[
                    facility
                ] = -adjustment  # NFAAtive adjustment (reduction in payment)
                self.agent_insight_context_manager.add_event(
                    f"CO2 Adjustment for {facility}",
                    f"Applied adjustment of {adjustment}",
                )

        self.agent_insight_context_manager.add_event(
            "CO2 Policy Application Complete",
            f"Applied CO2 policy adjustments to {len(adjustments)} facilities",
        )
        return adjustments
    ...
    def _get_remittance_table_names(self) -> Dict[str, str]:
        ...
        try:
            ...
            if not invoice_table_name or not subtotals_table_name:
                ...
                self.agent_insight_context_manager.add_event(
                    "Remittance Table Names Error", error_msg
                )
                raise ValueError(error_msg)

            self.logger.info(
                f"Successfully identified remittance tables: Invoice={invoice_table_name}, Subtotals={subtotals_table_name}"
            )
            ...
            self.logger.error(
                f"Error identifying remittance tables: {str(e)}", exc_info=True
            )
            self.agent_insight_context_manager.add_event(
