...
def parse_threshold(threshold_str: str) -> Decimal:
    ...
    try:
        ...
        raise ValueError(f"Invalid threshold value: {threshold_str}. Must be a positive number or percentage.")
...
class DatabaseDataCleaner:
    ...
    def clean_customer_data(customer_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'customer_id': DatabaseDataCleaner.clean_string(customer_data['customer_id']),
            'customer_name': DatabaseDataCleaner.clean_string(customer_data['customer_name']),
            'account_number': DatabaseDataCleaner.clean_string(customer_data['account_number'])
        }
    ...
    def clean_facility_data(facility_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'facility_id': DatabaseDataCleaner.clean_string(facility_data['facility_id']),
            'facility_name': DatabaseDataCleaner.clean_string(facility_data['facility_name']),
            'facility_type': DatabaseDataCleaner.clean_string(facility_data['facility_type']),
            'customer_id': DatabaseDataCleaner.clean_string(facility_data['customer_id'])
        }
    ...
    def clean_invoice_data(invoice_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            'invoice_number': DatabaseDataCleaner.clean_string(invoice_data['invoice_number']),
            'customer_id': DatabaseDataCleaner.clean_string(invoice_data['customer_id']),
            'facility_id': DatabaseDataCleaner.clean_string(invoice_data['facility_id']),
            'facility_type': DatabaseDataCleaner.clean_string(invoice_data['facility_type']),
            'service_type': DatabaseDataCleaner.clean_string(invoice_data.get('service_type')),
            'usage_unit': DatabaseDataCleaner.clean_string(invoice_data.get('usage_unit')),
