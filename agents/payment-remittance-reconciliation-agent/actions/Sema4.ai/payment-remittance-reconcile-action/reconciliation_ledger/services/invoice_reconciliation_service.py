...
class InvoiceReconciliationLedgerService:
    ...
    def analyze_payment_reconciliation(
        ...
        try:
            ...
            with self.context_manager.phase_context(
                ...
                match_results = self._match_payment_receivables(
                    payment_reference, threshold
                )

                if match_results.matching_status == "Match":
                    return ReconciliationResult(
                        status="MATCHED", customer_results=match_results
                    )
            ...
            self.logger.error(
                f"Reconciliation analysis failed: {str(e)}", exc_info=True
            )
            raise
        ...
        with self.loader.get_connection() as conn:
            ...
            for row in invoices:
                ...
                facility_totals[facility_type] = FacilityAmounts(
                    allocated=allocated, outstanding=ledger_net
                )
                    ...
                    discrepancies[facility_type] = FacilityDiscrepancy(
                        ...
                        components=FacilityComponentDetail(
                            ...
                            outstanding=FacilityComponents(
                                base=base, charges=charges, discounts=discounts
                            ),
                        ),
                    )
    ...
    def _analyze_invoice_discrepancies(
        ...
        self.logger.info(
            f"Starting invoice analysis for payment reference: {payment_reference}"
        )
        ...
        try:
            with self.loader.get_connection() as conn:
                ...
                for row in results:
                    ...
                    invoice_net = (invoice_amount + charges - discounts).quantize(
                        Decimal("0.01")
                    )
                    ...
                    difference = (allocated_amount - invoice_net).quantize(
                        Decimal("0.01")
                    )
                    if abs(difference) > threshold:
                        discrepancies.append(
                            InvoiceDiscrepancy(
                                ...
                                facility_id=row[1],
                                service_type=row[3],
                                discount_discrepancy=Decimal(str(row[6])) - discounts
                                if row[6] != discounts
                                else None,
                                charges_discrepancy=Decimal(str(row[5])) - charges
                                if row[5] != charges
                                else None,
                            )
                        )
    ...
    def _create_payment_record(self, conn, payment_data: Dict[str, Any]) -> str:
        ...
        total_payment = Decimal(str(payment_data["total_payment"])).quantize(
            Decimal("0.01")
        )
        total_invoice = Decimal(str(payment_data["total_invoice_amount"])).quantize(
            Decimal("0.01")
        )
        total_charges = Decimal(str(payment_data.get("total_charges", 0))).quantize(
            Decimal("0.01")
        )
        total_discounts = Decimal(str(payment_data.get("total_discounts", 0))).quantize(
            Decimal("0.01")
        )
    ...
    def _create_allocation_record(
        ...
        invoice_amount = Decimal(str(invoice["Invoice Amount"])).quantize(
            Decimal("0.01")
        )
        discounts = Decimal(str(invoice.get("Discounts Applied", 0))).quantize(
            Decimal("0.01")
        )
        charges = Decimal(str(invoice.get("Additional Charges", 0))).quantize(
            Decimal("0.01")
        )
