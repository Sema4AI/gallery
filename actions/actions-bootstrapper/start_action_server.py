...
def find_executable(executable_name):
    """
    Cross-platform function to find the full path of an executable.
    """
...
def start_action_server(action_package_path, port, secrets):
    ...
    action_server_executable = find_executable("action-server")
    if not action_server_executable:
        print("Error: action-server executable not found in system PATH.")
        return

    start_command = f"{action_server_executable} start -p {port} > {log_path} 2>&1"
