...
import pandas as pd
from utils import get_snowflake_connection, execute_query, is_running_in_spcs
import os
...
def list_stage_files(
    ...
    try:
        ...
        return Response(error=f"Error listing files in stage {database_name.value}.{schema_name.value}.{stage_name.value}: {str(e)}")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with get_snowflake_connection() as conn:
                ...
                if stage_path:
                    clean_path = stage_path.strip('/')
                    stage_file_path = f"{clean_path}/{unique_filename}" if clean_path else unique_filename
                else:
