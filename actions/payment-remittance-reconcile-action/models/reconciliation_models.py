    ...
    def _prepare_for_validation(data: Dict[str, Any]) -> Dict[str, Any]:
        if "agent_insight_context" in data and isinstance(
            ...
            data["agent_insight_context"] = (
                ReconciliationAgentInsightContext.model_validate(
                    data["agent_insight_context"]
                )
            )
        return data
...
class RemittanceFields(BaseModel):
    ...
    customer_name: str = Field(
        default="Sample Customer", description="Name of the customer making the payment"
    )
    customer_id: str = Field(
        default="CUS-00000", description="Unique identifier for the customer"
    )
    ...
    payment_reference: str = Field(
        default="PMT-00000", description="Unique reference number for the payment"
    )
    total_payment: Decimal = Field(
        default=Decimal("0.00"), description="Total amount of the payment"
    )
    total_invoice_amount: Decimal = Field(
        default=Decimal("0.00"), description="Total amount of all invoices being paid"
    )
    ...
    bank_account: str = Field(
        default="*****0000", description="Masked bank account number"
    )
    remittance_notes: Optional[str] = Field(
        default=None, description="Additional notes provided with the remittance"
    )
...
class PaymentMatchResult(BaseModel):
    ...
    payment_id: str = Field(
        default="PMT-00000", description="Unique identifier for the payment record"
    )
    matching_status: str = Field(default="Pending", description="Match or Mismatch")
    payment_amount: Decimal = Field(
        default=Decimal("0.00"), description="Total amount of the payment"
    )
    outstanding_balance: Decimal = Field(
        default=Decimal("0.00"), description="Total outstanding balance in AR system"
    )
    base_amount: Decimal = Field(
        default=Decimal("0.00"), description="Base amount before adjustments"
    )
    total_charges: Decimal = Field(
        default=Decimal("0.00"), description="Sum of all additional charges"
    )
    total_discounts: Decimal = Field(
        default=Decimal("0.00"), description="Sum of all applied discounts"
    )
...
class FacilityComponents(BaseModel):
    ...
    base: Decimal = Field(default=Decimal("0.00"), description="Base amount component")
    charges: Decimal = Field(
        default=Decimal("0.00"), description="Additional charges component"
    )
    discounts: Decimal = Field(
        default=Decimal("0.00"), description="Applied discounts component"
    )
...
class FacilityComponentDetail(BaseModel):
    ...
    allocated: FacilityComponents = Field(
        default_factory=FacilityComponents, description="Components of allocated amount"
    )
...
class FacilityAmounts(BaseModel):
    ...
    allocated: Decimal = Field(
        default=Decimal("0.00"), description="Amount allocated to the facility"
    )
    outstanding: Decimal = Field(
        default=Decimal("0.00"), description="Outstanding amount for the facility"
    )
...
class FacilityDiscrepancy(BaseModel):
    ...
    ledger_amount: Decimal = Field(
        default=Decimal("0.00"), description="Amount recorded in the ledger"
    )
    remittance_amount: Decimal = Field(
        default=Decimal("0.00"), description="Amount stated in the remittance"
    )
...
class FacilityAnalysisResult(BaseModel):
    ...
    facility_totals: Dict[str, FacilityAmounts] = Field(
        default_factory=dict, description="Totals by facility type"
    )
    discrepancies: Dict[str, FacilityDiscrepancy] = Field(
        default_factory=dict, description="Discrepancies found by facility type"
    )
    threshold: Decimal = Field(
        default=Decimal("0.01"), description="Threshold used for discrepancy detection"
    )
...
class InvoiceDetail(BaseModel):
    ...
    allocated_net: Decimal = Field(
        default=Decimal("0.00"), description="Net amount allocated to the invoice"
    )
    invoice_net: Decimal = Field(
        default=Decimal("0.00"), description="Net amount on the invoice"
    )
...
class InvoiceDiscrepancy(BaseModel):
    ...
    invoice_number: str = Field(
        default="INV-00000", description="Invoice reference number"
    )
    difference: Decimal = Field(
        default=Decimal("0.00"), description="Total discrepancy amount for the invoice"
    )
    facility_id: str = Field(
        default="FAC-00000", description="Facility ID associated with the invoice"
    )
    service_type: str = Field(
        default="Unknown", description="Type of service on the invoice"
    )
    discount_discrepancy: Optional[Decimal] = Field(
        default=None, description="Difference in discount amounts, if applicable"
    )
    charges_discrepancy: Optional[Decimal] = Field(
        default=None, description="Difference in additional charges, if applicable"
    )
...
class InvoiceAnalysisResult(BaseModel):
    ...
    invoice_details: Dict[str, InvoiceDetail] = Field(
        default_factory=dict, description="Details for all analyzed invoices"
    )
    discrepancies: List[InvoiceDiscrepancy] = Field(
        default_factory=list, description="List of found invoice discrepancies"
    )
    threshold: Decimal = Field(
        default=Decimal("0.01"), description="Threshold used for discrepancy detection"
    )
...
class ReconciliationSummaryMetrics(BaseModel):
    ...
    total_discrepancy: Decimal = Field(
        default=Decimal("0.00"), description="Total amount of all discrepancies found"
    )
    facility_types_with_issues: int = Field(
        default=0, description="Number of facility types with discrepancies"
    )
    invoices_with_issues: int = Field(
        default=0, description="Number of invoices with discrepancies"
    )
...
class ReconciliationResult(BaseModel):
    ...
    status: str = Field(default="PENDING", description="MATCHED or DISCREPANCY_FOUND")
    ...
    facility_results: Optional[FacilityAnalysisResult] = Field(
        default=None, description="Results of facility-level analysis, if performed"
    )
    invoice_results: Optional[InvoiceAnalysisResult] = Field(
        default=None, description="Results of invoice-level analysis, if performed"
    )
    summary_metrics: Optional[ReconciliationSummaryMetrics] = Field(
        default=None, description="Summary metrics of the reconciliation process"
    )
