...
from models.reconciliation_models import (
    DiscrepancySummary,
    FacilityAmountSummary,
    InvoiceDiscrepancyDetail,
    ProcessingMetrics,
    ReconciliationPhase,
...
class InvoiceReconciliationLedgerService:
    ...
    def store_payment_with_allocations(
        self, payment_data: Dict[str, Any], invoices: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Store payment and its allocations."""
        with self.context_manager.phase_context(
            ReconciliationPhase.PAYMENT_DATA_LOADING
        ):
            ...
            try:
                ...
                self.context_manager.update_metrics(
                    {
                        ...
                        "unique_facility_types": len(
                            set(inv["Facility Type"] for inv in invoices)
                        ),
                        "unique_service_types": len(
                            set(
                                inv["Service Type"]
                                for inv in invoices
                                if inv.get("Service Type")
                            )
                        ),
                        "document_metrics": {
                ...
                with self.loader.get_connection() as conn:
                    ...
                    total_payment = self._parse_monetary_value(
                        payment_data["total_payment"]
                    )  # Net payment
                    total_invoice = self._parse_monetary_value(
                        payment_data["total_invoice_amount"]
                    )  # Gross AR
                    total_discounts = self._parse_monetary_value(
                        payment_data.get("total_discounts", 0)
                    )
                    total_charges = self._parse_monetary_value(
                        payment_data.get("total_charges", 0)
                    )

                    # Calculate net amount using DecimalHandler
                    net_amount = DecimalHandler.round_decimal(
                        total_invoice - total_discounts + total_charges
                    )
                    ...
                    for invoice in invoices:
                        ...
                        amount_paid = self._parse_monetary_value(
                            invoice["Amount Paid"]
                        )  # Net amount
                        invoice_amount = self._parse_monetary_value(
                            invoice["Invoice Amount"]
                        )  # Gross amount
                        discounts = self._parse_monetary_value(
                            invoice.get("Discounts Applied", 0)
                        )
                        charges = self._parse_monetary_value(
                        ...
                        if facility_type not in facility_metrics["invoice_counts"]:
                            ...
                            facility_metrics["amount_totals"][facility_type] = (
                                DecimalHandler.from_str("0")
                            )

                        facility_metrics["invoice_counts"][facility_type] += 1
                        facility_metrics["amount_totals"][facility_type] = (
                            DecimalHandler.round_decimal(
                                facility_metrics["amount_totals"][facility_type]
                                + amount_paid
                            )
                        )
                        ...
                        if invoice.get("Service Type"):
                            facility_metrics["service_types"].add(
                    ...
                    for facility, amount in facility_metrics["amount_totals"].items():
                        percentage = DecimalHandler.calc_percentage(
                            amount / total_payment, DecimalHandler.from_str("100")
                        )

                        facility_distribution[facility] = {
                            "invoice_count": facility_metrics["invoice_counts"][
                                facility
                            ],
                    ...
                    self.context_manager.update_metrics(
                        {
                            "facility_metrics": {
                                ...
                                "amount_totals": {
                                    k: str(v)
                                    for k, v in facility_metrics[
                                        "amount_totals"
                                    ].items()
                                },
                                "service_types": sorted(
                                    facility_metrics["service_types"]
                                ),
    ...
    def analyze_payment_reconciliation(
        self, payment_reference: str, threshold: Decimal
    ) -> ReconciliationResult:
        ...
        try:
            with self.loader.get_connection() as conn:
                ...
                with self.context_manager.phase_context(
                    ReconciliationPhase.PAYMENT_MATCHING
                ):
                    self.context_manager.add_event(
                    ...
                    self.context_manager.update_metrics(
                        {
                            "payment_totals": {
                                "customer_payment": str(
                                    DecimalHandler.round_decimal(customer_payment)
                                ),
                                "ar_gross": str(DecimalHandler.round_decimal(ar_gross)),
                                "ar_discounts": str(
                                    DecimalHandler.round_decimal(ar_discounts)
                                ),
                                "ar_net": str(
                                    DecimalHandler.round_decimal(total_ar_net)
                                ),
                            },
                            "document_metrics": {
                    ...
                    total_difference = DecimalHandler.round_decimal(
                        customer_payment - total_ar_net
                    )
                    ...
                    self.context_manager.update_metrics(
                        {
                            "matching_results": {
                                "total_difference": str(
                                    DecimalHandler.round_decimal(total_difference)
                                ),
                                "has_discrepancy": has_discrepancy,
                                "threshold": str(
                                    DecimalHandler.round_decimal(threshold)
                                ),
                            }
                        }
                    )
                ...
                with self.context_manager.phase_context(
                    ReconciliationPhase.FACILITY_TYPE_RECONCILIATION
                ):
                    self.context_manager.add_event(
                    ...
                    facility_summaries = self._calculate_facility_summaries(
                        invoice_data, threshold
                    )

                    facility_metrics = {
                        "total_facilities": len(facility_summaries),
                        "facilities_with_discrepancies": len(
                            [f for f in facility_summaries if f.has_discrepancy]
                        ),
                        "facility_details": {
                            f.facility_type: {
                                "remittance_amount": str(
                                    DecimalHandler.round_decimal(f.remittance_amount)
                                ),
                                "ar_amount": str(
                                    DecimalHandler.round_decimal(f.ar_system_amount)
                                ),
                                "difference": str(
                                    DecimalHandler.round_decimal(f.difference)
                                ),
                ...
                with self.context_manager.phase_context(
                    ReconciliationPhase.INVOICE_LEVEL_RECONCILIATION
                ):
                    self.context_manager.add_event(
                    ...
                    invoice_metrics = {
                        ...
                        "discrepancy_summary": {"by_facility": {}, "by_service": {}},
                    }
                    ...
                    for invoice in invoice_data:
                        ...
                        if abs(difference) > threshold:
                            ...
                            if (
                                facility
                                not in invoice_metrics["discrepancy_summary"][
                                    "by_facility"
                                ]
                            ):
                                invoice_metrics["discrepancy_summary"]["by_facility"][
                                    facility
                                ] = 0
                            invoice_metrics["discrepancy_summary"]["by_facility"][
                                facility
                            ...
                            if (
                                ...
                                invoice_metrics["discrepancy_summary"]["by_service"][
                                    service
                                ] = 0
                            invoice_metrics["discrepancy_summary"]["by_service"][
                                service
                    ...
                    self.context_manager.add_event(
                        ...
                        "Completed invoice-level reconciliation",
                        {
                            ...
                            "discrepancies_found": invoice_metrics[
                                "invoices_with_discrepancies"
                            ],
                        },
                    )
                ...
                with self.context_manager.phase_context(
                    ReconciliationPhase.PAYMENT_RECONCILIATION
                ):
                    self.context_manager.add_event(
                    ...
                    if has_discrepancy:
                        all_matched = (
                            False  # Also set to False if total amount has discrepancy
                        )
                    ...
                    result = ReconciliationResult(
                        ...
                        discrepancy_summary=DiscrepancySummary(
                            ...
                            affected_service_types=sorted(
                                set(d.service_type for d in discrepancy_details)
                            ),
                        )
                        if has_discrepancy
                        else None,
                        invoice_discrepancies=discrepancy_details
                        if has_discrepancy
                        else None,
                    ...
                    self.context_manager.update_metrics(
                        {
                            "final_reconciliation": {
                                ...
                                "affected_facilities": result.discrepancy_summary.affected_facility_count
                                if result.discrepancy_summary
                                else 0,
                                "affected_invoices": result.discrepancy_summary.affected_invoice_count
                                if result.discrepancy_summary
                                else 0,
                    ...
                    self.context_manager.add_event(
                        ...
                        {"final_status": result.status, "match_achieved": all_matched},
                    )
    ...
    def _calculate_facility_summaries(
        self, invoice_data: List[Dict], threshold: Decimal
    ...
    def _calculate_processing_metrics(self, invoice_data: List[Dict]) -> Dict[str, Any]:
        ...
        service_types = sorted(
            set(inv["service_type"] for inv in invoice_data if inv["service_type"])
        )
    ...
    def _create_payment_record(self, conn, payment_data: Dict[str, Any]) -> str:
        ...
        payment_id = (
            f"PMT-{payment_data['payment_date']}-{payment_data['payment_reference']}"
        )

        # Convert monetary values with proper decimal handling
        total_payment = DecimalHandler.from_str(
            str(payment_data["total_payment"])
        )  # Net amount
        total_invoice = DecimalHandler.from_str(
            str(payment_data["total_invoice_amount"])
        )  # Gross AR
        total_charges = DecimalHandler.from_str(
            str(payment_data.get("total_charges", 0))
        )
        total_discounts = DecimalHandler.from_str(
            str(payment_data.get("total_discounts", 0))
        )
    ...
    def _create_allocation_record(
        self, conn, customer_id: str, payment_id: str, invoice: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        result = conn.execute(
            query, [customer_id, invoice["Invoice Number"]]
        ).fetchone()
        if not result:
            raise ValueError(
                f"Invoice {invoice['Invoice Number']} not found for customer {customer_id}"
            )
        ...
        amount_applied = DecimalHandler.from_str(
            str(invoice["Amount Paid"])
        )  # Net amount
        invoice_amount = DecimalHandler.from_str(
            str(invoice["Invoice Amount"])
        )  # Gross amount
