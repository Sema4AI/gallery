...
class DatabaseSetupGenerator:
    ...
    def _calculate_database_amounts(
        self,
        invoices: List[Dict],
        discrepancy_config: Optional[Dict]
        ...
        original_totals = {}
        for facility_type in facility_types:
            facility_invoices = [inv for inv in invoices if inv['Facility Type'] == facility_type]
            total = DecimalHandler.from_str('0')
            for inv in facility_invoices:
                amount = DecimalHandler.from_str(str(inv['Amount Due']).replace(',', ''))
        ...
        for invoice in invoices:
            ...
            base_amount = DecimalHandler.from_str(str(invoice['Amount Due']).replace(',', ''))
            
            adjusted_amount = base_amount
            if facility_type in adjustments:
                adjustment = adjustments[facility_type]
                if isinstance(adjustment, dict):
                    if adjustment['condition'] == 'water_service' and invoice['Service Type'] == 'Water':
                        adjusted_amount = DecimalHandler.round_decimal(base_amount * adjustment['factor'])
                else:
                    adjusted_amount = DecimalHandler.round_decimal(base_amount * adjustment)
            ...
            total_db_amount = DecimalHandler.round_decimal(total_db_amount + adjusted_amount)
    ...
    def debug_generate_invoice_records(self, 
                                    invoices: List[Dict],
                                    facility_type: str = "Greenhouse Complexes") -> None:
