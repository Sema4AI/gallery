...
# Import from processor or core for use case-specific constants
from validation.validation_constants import CONFIG_MULTIPLE_FIELD_MAPPING_KEY
...
class FieldTypes:
    ...
    MONETARY_FIELDS = ['Total Payment Amount', 'Total Discounts Applied', 'Total Payment Paid', 'Total Additional Charges', 'Invoice Amount', 'Amount Paid', 'Additional Charges', 'Discounts Applied', 'Total Invoice Amount'  ]
...
class DocumentIntelligenceUtility:
    ...
    def __init__(self, document_type: DocType, document_format: DocumentFormat, insight_tracker: ValidationAgentContextManager):
        """
    ...
    def standardize_non_tabular_to_doc_type_schema(self, non_tabular_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in self._document_type.non_tbl_fields:
            ...
            if doc_type_field in non_tabular_data:
                standardized_non_tabular[doc_type_field] = non_tabular_data[doc_type_field]
    ...
    def standardize_tabular_to_doc_type_schema(self, data: Dict[str, List[Dict[str, Any]]], table_name: str) -> Dict[str, pd.DataFrame]:
        ...
        tbl_fields_for_table = [field for field in self._document_type.tbl_fields if field.name == table_name]
        ...
        for table_name, table_data in data.items():
            ...
            for col in df.columns:
                mapped_col = self.map_format_to_type_field(col)  # Keep the dynamic approach here
                if mapped_col:
    ...
    def map_format_to_type_field(self, format_field: str, multiple_mappings: Dict[str, List[str]] = None) -> Optional[str]:
        ...
        if multiple_mappings:
            mapped_field_doc_type = self.find_doc_type_mapped_to_multiple_formats(multiple_mappings, format_field)
            if mapped_field_doc_type:
                ...
                self.insight_tracker.add_event(
                    ...
                    {"format_field": format_field, "type_field": mapped_field_doc_type, "multiple_mappings_map": multiple_mappings}
                )
    ...
    def find_doc_type_mapped_to_multiple_formats(self, mapping: Dict[str, List[str]], format_mapping: str) -> Optional[str]:
        """
    ...
    def validate_and_clean_tabular(self, data: List[Dict[str, Any]], table_type: str, exclude_field_validate_list: List[str] = []) -> Tuple[pd.DataFrame, ValidationResults]:
        ...
        for doc_type_field in table_fields_for_type:
            for column in doc_type_field.table_definition:
                ...
                if field_name in exclude_field_validate_list:
                    self.insight_tracker.add_event("Field Validation Skip", f"Skipping validation for field: {field_name}")
                    continue
                ...
                if field_name not in df.columns:
                    if column.requirement == "Required":
                        ...
                        self.insight_tracker.add_event("Missing Required Field", 
                                                    error_message,
                                                    {"table_type": table_type, "field_name": field_name})
                else:
                    df = self.convert_field_based_on_type(df, field_name, field_name, validation_results, table_type)

        # Validate that all required fields are present
        required_fields = [column.column for doc_type_field in table_fields_for_type 
                        for column in doc_type_field.table_definition if doc_type_field.name == table_type 
                        if column.requirement == "Required" and column.column not in exclude_field_validate_list]
        ...
        self.insight_tracker.add_event(f"{table_type} Validation Complete", 
    ...
    def validate_and_clean_non_tabular(self, data: Dict[str, Any], exclude_field_validate_list: List[str] = []) -> Tuple[Dict[str, Any], ValidationResults]:
        ...
        for field_name, value in data.items():
            ...
            if doc_type_field.requirement == "Required" and (value is None or value == ""):
                validation_results.add_result(
                ...
                cleaned_value = self.convert_single_field(value, field_name, validation_results)
                cleaned_data[field_name] = cleaned_value

        self.insight_tracker.add_event("Non-tabular Validation Complete", 
    ...
    def convert_field_based_on_type(self, df: pd.DataFrame, format_field: str, field_name: str, validation_results: ValidationResults, table_type: str) -> pd.DataFrame:
        try:
            ...
            changes = (original_values != df[format_field]) & (~original_values.isna() | ~df[format_field].isna())
            if changes.any():
                change_details = df[changes].groupby('Page').apply(lambda x: x.index.tolist()).to_dict()
            ...
            if errors.any():
                error_details = df[errors].groupby('Page').apply(lambda x: x.index.tolist()).to_dict()
                ...
                self.insight_tracker.add_event("Field Conversion Error", 
                                            f"Error converting field: '{format_field}' (mapped to '{field_name}')",
                                            {"table_type": table_type, "errors_by_page": error_details})

        except Exception as e:
            self._logger.error(f"Error in convert_field_based_on_type: {str(e)}", exc_info=True)
            validation_results.add_result(
                rule_id=f"{RULE_PREFIX_FOR_INVALID_FIELD_FORMAT_FIELD}{field_name.upper().replace(' ', '_')}",
                passed=False,
                message=f"Error processing {table_type} Column '{format_field}' (mapped to '{field_name}'): {str(e)}",
                severity=ValidationSeverity.ERROR,
                details={"format_field": format_field, "mapped_field": field_name, "table_type": table_type, "error": str(e)}
            )
            self.insight_tracker.add_event("Field Conversion Error", 
                                        f"Error processing field: '{format_field}' (mapped to '{field_name}')",
                                        {"table_type": table_type, "format_field": format_field, "mapped_field": field_name, "error": str(e)})
        
        return df
            
    def convert_single_field(self, value: Any, field_name: str, validation_results: ValidationResults) -> Any:
        try:
    ...
    def get_unmapped_fields(self) -> Tuple[List[str], List[str]]:
        ...
        """
        unmapped_non_table = [field.name for field in self._document_type.non_tbl_fields if not self.has_field_mapping(field.name)]
        
        unmapped_table = []
        for table in self._document_type.tbl_fields:
            unmapped_table.extend([field.column for field in table.table_definition if not self.has_field_mapping(field.column)])
        
        return unmapped_non_table, unmapped_table

    @ValidationAgentContextManager.track_method_execution(method_name="get_mapped_fields")
    def get_mapped_fields(self) -> Tuple[List[str], List[str]]:
        ...
        """
        mapped_non_table = [field.name for field in self._document_type.non_tbl_fields if self.has_field_mapping(field.name)]
        
        mapped_table = []
        for table in self._document_type.tbl_fields:
            mapped_table.extend([field.column for field in table.table_definition if self.has_field_mapping(field.column)])
    ...
    def map_type_to_format_field(self, type_field: str) -> Optional[str]:
        ...
        try:
            ...
            self._logger.error(f"Error in map_type_to_format_field: {str(e)}", exc_info=True)
            return None
    ...
    def get_table_info(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for table in self._document_type.tbl_fields:
                ...
                required_columns = [field.column for field in table.table_definition 
                                if field.requirement == "Required"]
                optional_columns = [field.column for field in table.table_definition 
    ...
    def get_table_columns(self, table_name: str) -> Dict[str, List[str]]:
        ...
        """
        self.insight_tracker.add_event(
            "Table Columns Retrieval",
            f"Retrieving columns for table: {table_name}"
        )
        
        try:
            # Find the specified table
            table = next((table for table in self._document_type.tbl_fields 
            ...
            self._logger.error(f"Error retrieving table columns: {str(e)}", exc_info=True)
            self.insight_tracker.add_event(
                "Table Columns Retrieval Error",
