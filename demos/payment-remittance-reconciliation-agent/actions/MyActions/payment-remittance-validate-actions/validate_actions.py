...
def run_remittance_extraction(remittance_id: str) -> ActionResponse:
    ...
    try:
        ...
        message = (
            f"An error occurred when running extraction on remittance document {remittance_id}: {str(e)}",
        )
        return handle_action_exception(
            remittance_id, e, agent_context_manager, custom_message=message
        )
...
def run_remittance_validation(remittance_id: str) -> ActionResponse:
    ...
    try:
        ...
        logger.info(
            f"Returning action response from : {serialize_any_object_safely(action_response)}"
        )
...
@action
def update_work_item_with_unexpected_exeception(
    ...
    """
    return _update_work_item_status(
        remittance_id, "ERROR", error_summary, error_detailed_report
    )
