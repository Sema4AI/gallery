...
# Configure logging
logger = configure_logging(logger_name=__name__, log_config_path="logging-validate-actions.conf")
...
def get_remittance_work_item_for_validation(remittance_id: str)  -> DocumentWorkItem:
    ...
    try:
        ...
        logger.error(f"Error occurred while fetching remittance work item for ID: {remittance_id}", exc_info=True)
        raise e
...
def run_remittance_extraction(remittance_id: str) -> ActionResponse:
    ...
    try:
        ...
        if extraction_status == "Failed":
            ...
            logger.error(f"Extraction failed for document: {document_name}. Please review the errors and take corrective actions.")
            logger.error(f"Extraction Errors: {agent_context.get_validation_results(ProcessingPhase.EXTRACTION)}")
...
def run_remittance_transformation(remittance_id: str) -> ActionResponse:
    ...
    try:
        ...
        transformation_status = agent_context.get_phase_status(ProcessingPhase.TRANSFORMATION)
        if transformation_status == "Failed":
            ...
            logger.error(f"Transformation failed for document: {document_name}. Please review the errors and take corrective actions.")
            logger.error(f"Transformation Errors: {agent_context.get_validation_results(ProcessingPhase.TRANSFORMATION)}")
            _add_document_format_to_context(remittance_work_item, doc_intel_client, agent_context_manager)
        else:
            status = ActionStatus.SUCCESS
...
def run_remittance_validation(remittance_id: str) -> ActionResponse:
    ...
    try:
        ...
        if validation_final_result.validation_status == ValidationStatus.VALIDATION_PASSED:
            status = ActionStatus.SUCCESS
            message = f'Validation completed successfully for document: {document_name}. All checks passed.'
        else:
            status = ActionStatus.FAILURE
            _add_document_format_to_context(remittance_work_item, doc_intel_client, agent_context_manager)
            validation_results = agent_context.get_validation_results(ProcessingPhase.VALIDATION)
            ...
            logger.error(f"Validation failed for document: {document_name}. Failed rules: {[r.rule_id for r in failed_rules]}")
...
def _update_work_item_status(remittance_id: str, status: str, message: str, details: Optional[str]) -> ActionResponse:
    ...
    try:
        ...
        doc_intel_client.work_items_complete_stage(remittance_id, status, message, details)
        
        logger.debug(f"Successfully updated status for remittance ID: {remittance_id} to {status}")
        ...
        logger.error(f"Error occurred while updating remittance processing status: {str(e)}", exc_info=True)
        return ActionResponse(
            status=ActionStatus.FAILURE,
...
def clean_pydantic_object(obj: BaseModel) -> BaseModel:
    ...
    for field_name, field_value in obj.dict().items():
        if isinstance(field_value, BaseModel):
            ...
            cleaned_data[field_name] = [clean_pydantic_object(item) if isinstance(item, BaseModel) else clean_value(item) for item in field_value]
        elif isinstance(field_value, dict):
            cleaned_data[field_name] = {k: clean_pydantic_object(v) if isinstance(v, BaseModel) else clean_value(v) for k, v in field_value.items()}
        else:
...
def handle_action_exception(remittance_id: str,  e: Exception, agent_context_manager: Optional[ValidationAgentContextManager] = None, custom_message: Optional[str] = None) -> ActionResponse:
    """
    ...
    if agent_context_manager and agent_context_manager.get_agent_context():
        agent_context_manager.add_error("Error Occurred", f"An error occurred: {str(e)}")
        agent_context_manager.store_context()
...
def _add_document_format_to_context(remittance_work_item: DocumentWorkItem, doc_intel_client: DocumentIntelligenceClient, agent_context_manager):
    """
