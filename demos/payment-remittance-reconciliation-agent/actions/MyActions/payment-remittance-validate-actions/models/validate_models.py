...
class ProcessingEvent(BaseModel):
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="The time when the event occurred"
    )
    event_type: Optional[str] = Field(
        None,
        description="The type of event (e.g., 'Table Extraction', 'Validation Check')",
    )
    description: Optional[str] = Field(
        None, description="A detailed description of the event"
    )
    details: Optional[Dict[str, Any]] = Field(
        None, description="Additional details specific to the event"
    )
...
class DataMetrics(BaseModel):
    metrics: Dict[str, Any] = Field(
        default_factory=dict, description="A dictionary to store various metrics"
    )
...
class PreprocessingSummary(BaseModel):
    page_patterns_compiled: Optional[bool] = Field(
        False, description="Indicates if page patterns were successfully compiled"
    )
    header_identification_method: Optional[str] = Field(
        None, description="The method used to identify headers"
    )
    charge_type_sections_identified: Optional[List[str]] = Field(
        default_factory=list,
        description="List of charge type sections identified in the document",
    )
...
class TableExtractionMetrics(BaseModel):
    tables_per_page: Optional[Dict[int, int]] = Field(
        default_factory=dict,
        description="A mapping of page numbers to the number of tables extracted from each",
    )
    empty_columns_dropped: Optional[List[str]] = Field(
        default_factory=list,
        description="List of column names that were dropped due to being empty",
    )
    columns_renamed: Optional[Dict[str, str]] = Field(
        default_factory=dict,
        description="A mapping of original column names to their renamed versions",
    )
    total_raw_tables: Optional[int] = Field(
        default=0, description="Total number of raw tables extracted"
    )
    total_raw_rows: Optional[int] = Field(
        default=0, description="Total number of raw rows extracted"
    )
    extracted_tables: Optional[int] = Field(
        default=0, description="Total number of tables successfully extracted"
    )
    extracted_rows: Optional[int] = Field(
        default=0, description="Total number of rows successfully extracted"
    )

    # New fields for invoice details
    invoice_details_extracted_tables: Optional[int] = Field(
        default=0, description="Number of invoice detail tables extracted"
    )
    invoice_details_extracted_rows: Optional[int] = Field(
        default=0, description="Number of rows extracted from invoice detail tables"
    )

    # New fields for summary tables
    summary_extracted_tables: Optional[int] = Field(
        default=0, description="Number of summary tables extracted"
    )
    summary_extracted_rows: Optional[int] = Field(
        default=0, description="Number of rows extracted from summary tables"
    )
...
class ValidationCheckpoint(BaseModel):
    stage: Optional[str] = Field(
        None, description="The stage of processing where the validation occurred"
    )
    ...
    checks_passed: Optional[List[str]] = Field(
        default_factory=list, description="List of validation checks that passed"
    )
    checks_failed: Optional[List[str]] = Field(
        default_factory=list, description="List of validation checks that failed"
    )
...
class TransformationStep(BaseModel):
    step_name: Optional[str] = Field(
        None, description="The name of the transformation step"
    )
    rows_affected: Optional[int] = Field(
        0, description="The number of rows affected by this transformation"
    )
    calculation_performed: Optional[str] = Field(
        None, description="Description of any calculation performed during this step"
    )
...
class DataQualityIndicators(BaseModel):
    completeness: Optional[float] = Field(
        0.0, ge=0, le=1, description="Measure of data completeness (0-1)"
    )
    consistency: Optional[float] = Field(
        0.0, ge=0, le=1, description="Measure of data consistency (0-1)"
    )
    accuracy: Optional[float] = Field(
        0.0, ge=0, le=1, description="Measure of data accuracy (0-1)"
    )
...
class ValidationResult(BaseModel):
    rule_id: Optional[str] = Field(
        None, description="The unique identifier of the validation rule"
    )
    passed: Optional[bool] = Field(
        None, description="Indicates whether the validation rule passed"
    )
    message: Optional[str] = Field(
        None, description="The message describing the validation result"
    )
    severity: Optional[ValidationSeverity] = Field(
        None, description="The severity level of the validation result"
    )
    details: Optional[Dict[str, Any]] = Field(
        None, description="Additional details about the validation result"
    )


class ValidationResults(BaseModel):
    rules_passed: int = Field(
        0, description="The number of validation rules that passed"
    )
    rules_failed: int = Field(
        0, description="The number of validation rules that failed"
    )
    results: List[ValidationResult] = Field(
        default_factory=list, description="The list of individual validation results"
    )
    ...
    def get_overall_status(self) -> bool:
        ...
        return not any(
            result.severity == ValidationSeverity.ERROR for result in self.results
        )
...
class ProcessingSummary(BaseModel):
    phase: Optional[ProcessingPhase] = Field(
        None, description="The current processing phase"
    )
    start_time: Optional[datetime] = Field(
        None, description="The start time of the processing phase"
    )
    end_time: Optional[datetime] = Field(
        None, description="The end time of the processing phase"
    )
    processing_events: Optional[List[ProcessingEvent]] = Field(
        default_factory=list,
        description="A list of processing events that occurred during this phase",
    )
    data_metrics: Optional[DataMetrics] = Field(
        default_factory=DataMetrics,
        description="Metrics about the data processed in this phase",
    )
    preprocessing_summary: Optional[PreprocessingSummary] = Field(
        None, description="Summary of the preprocessing steps"
    )
    table_extraction_metrics: TableExtractionMetrics = Field(
        default_factory=TableExtractionMetrics,
        escription="Metrics related to table extraction",
    )
    transformation_steps: Optional[List[TransformationStep]] = Field(
        default_factory=list, description="List of transformation steps performed"
    )
    performance_metrics: Optional[Dict[str, float]] = Field(
        default_factory=dict,
        description="Performance metrics for various processing steps",
    )
    data_quality_indicators: Optional[DataQualityIndicators] = Field(
        default_factory=DataQualityIndicators,
        description="Indicators of data quality for this phase",
    )
...
class ProcessingContext(BaseModel):
    document_id: Optional[str] = Field(
        None, description="The unique identifier of the document being processed"
    )
    document_name: Optional[str] = Field(
        None, description="The name of the document being processed"
    )
    processing_phase: Optional[ProcessingPhase] = Field(
        None, description="The current processing phase"
    )
    summary: Optional[ProcessingSummary] = Field(
        default_factory=ProcessingSummary,
        description="A summary of the processing for this phase",
    )
    validation_results: Optional[ValidationResults] = Field(
        default_factory=ValidationResults,
        description="Results of validation checks, if applicable",
    )
    configuration_used: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Configuration settings used for this processing phase",
    )
    additional_context: Optional[Dict[str, Any]] = Field(
        default_factory=dict,
        description="Any additional context relevant to this processing phase",
    )
...
class AgentInsightContext(BaseModel):
    ...
    extraction_context: Optional[ProcessingContext] = Field(
        None, description="Context for the extraction phase"
    )
    transformation_context: Optional[ProcessingContext] = Field(
        None, description="Context for the transformation phase"
    )
    validation_context: Optional[ProcessingContext] = Field(
        None, description="Context for the validation phase"
    )
    overall_processing_time: Optional[float] = Field(
        0.0, description="Total time taken for all processing phases in seconds"
    )
    overall_status: Optional[str] = Field(
        None,
        description="Overall status of the document processing (e.g., 'Completed', 'Failed')",
    )
    ...
    def get_data_quality_indicators(
        self, phase: ProcessingPhase
        ...
        context = getattr(self, f"{phase.value.lower()}_context")
        return (
            context.summary.data_quality_indicators
    ...
    def get_error_results(
        self, phase: Optional[ProcessingPhase] = None
    ...
    def get_processing_time(self, phase: ProcessingPhase) -> Optional[float]:
        ...
        if (
            context
            and context.summary
            and context.summary.start_time
            and context.summary.end_time
        ):
            return (
    ...
    def model_dump_json(self, **kwargs):
        ...
        """
        return json.dumps(
            self.model_dump(exclude_none=True), default=self._json_default, **kwargs
        )
...
class PhaseResult(BaseModel):
    agent_insight_context: Optional[AgentInsightContext] = Field(
        None, description="The context of the agent insight"
    )
    ...
    ] = Field(None, description="The content of the phase result")

    model_config = ConfigDict(json_encoders={Series: lambda v: v.to_list()})


class ExtractionResult(PhaseResult):
    document_content: Optional[ExtractedDocumentContent] = Field(
        None, description="The content of the extraction result"
    )


class TransformationResult(PhaseResult):
    document_content: Optional[TransformedDocumentContent] = Field(
        None, description="The content of the transformation result"
    )
...
class ValidationFinalResult(PhaseResult):
    ...
    validation_status: Optional[ValidationStatus] = Field(
        None, description="The status of the validation"
    )
    document_content: Optional[ComputedDocumentContent] = Field(
        None, description="The content of the validation final result"
    )
...
class ActionResponse(BaseModel):
    ...
    message: str = Field(
        ..., description="A brief summary or instruction for the agent"
    )
    agent_insight_context: Optional[AgentInsightContext] = None
    additional_data: Optional[Dict[str, Any]] = Field(
        None, description="Any additional data that might be relevant"
    )
    ...
    def _depth_limited_dump(cls, obj, current_depth=0, max_depth=10):
        ...
        if isinstance(obj, BaseModel):
            ...
            return [
                cls._depth_limited_dump(item, current_depth + 1, max_depth)
                for item in obj
            ]
    ...
    def _prepare_for_validation(data: Dict[str, Any]) -> Dict[str, Any]:
        if "agent_insight_context" in data and isinstance(
            data["agent_insight_context"], dict
        ):
            data["agent_insight_context"] = AgentInsightContext.model_validate(
                data["agent_insight_context"]
            )
        return data
